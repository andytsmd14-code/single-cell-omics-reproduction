...
Personal reproduction project (independent study, not for course credit) based on:
...
| Script | Input | Output |
|---|---|---|
| `05_taiwan_reference.py` | Taiwan HPA cancer registry CSV (dataset 6399) | `taiwan_reference_clean_en.csv` |
...
- Chi-square tests confirm the paper's core finding: European ancestry is
  ...
  cancer types (HTAN) relative to reference populations.
- A newly reproduced result not shown in the original paper's main figures:
  Figure 1D (HCA sex composition vs. global reference) also shows a
  statistically significant, if modest, deviation from a 50/50 split
  (chi2 = 7.09, p = 0.0078).
